...
;系统控制快外设SCBS地址范围：0xE000ED00-0xE000ED3F
...
rt_hw_context_switch_to	PROC
	...
	LDR         r1, =rt_thread_switch_interrupt_flag     ;将rt_thread_interrupt_flag的地址加载到r1
	MOV         r0, #1                                   ;配置r0为1
	STR         r0, [r1]                                 ;将r0的值存储到rt_thread_interrupt_flag
...
PendSV_Handler	PROC
	...
	LDR			r1, [r0]                                    ; 加载rt_thread_interrupt_flag的值到r1
	CBZ			r1, pendsv_exit                             ; 判断r1是否为0，为0则跳转到pendsv_exit
	
	; r1不为0则清0
	MOV			r1, #0x00                                   
	STR			r1, [r0]                                    ; 将r1的值储存到rt_thread_switch_interrupt_flag，即清0
...
; ======================================上文保存=======================================================================================
	...
	LDR			r0, [r0]                                     ; 加载r0指向值到r0,即r0=rt_inerrupt_from_thread
	STR			r1, [r0]                                     ; 将r1的值存储到r0，即更新线程栈sp
	
;=======================================下文切换========================================================================================
switch_to_thread
	LDR			r1, =rt_interrupt_to_thread                  ; 加载rt_interrupt_thread的地址到r1
	                                                         ; rt_interrupt_thread是一个全局变量，里面存的是线程栈指针SP的指针
...
pendsv_exit
	...
	BX			lr                                           ; 异常返回，这个时候任务堆栈中的剩下的内容将会自动加载到xPSR, PC（任务入口地址），R14, R12, R3, R2, R1, R0 （任务形参）
	                                                         ; 同时PSP的值也将更新，即指向任务堆的栈顶。在ARMC3中，堆是由高地址向低地址生长的。
